...
public class ColorTextView extends View {
    ...
    public ColorTextView(Context context, AttributeSet attrs, int defStyle){
        ...
        for (int i = 0; i < n; i++) {
            ...
            } else if (attr == R.styleable.ColorTextView_ctvTextColor){// 默认颜色设置为黑色
                mTitleTextColor = a.getColor(attr, Color.BLACK);
                
            } else if (attr == R.styleable.ColorTextView_ctvTextSize){// 默认设置为16sp，TypeValue也可以把sp转化为px
                mTitleTextSize = a.getDimensionPixelSize(attr, (int) TypedValue.applyDimension(
                    TypedValue.COMPLEX_UNIT_SP, 16, getResources().getDisplayMetrics()));
                
            } else if (attr == R.styleable.ColorTextView_ctvBackground){//默认为白色
